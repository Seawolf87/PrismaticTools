...
namespace PrismaticTools.Framework
{
    ...
    public static class SprinklerInitializer
    {
        ...
        {
            ModEntry.ModHelper.Events.GameLoop.DayStarted += TimeEvents_DayStarted;
            ModEntry.ModHelper.Events.GameLoop.SaveLoaded += SaveEvents_Loaded;
            ModEntry.ModHelper.Events.World.ObjectListChanged += LocationEvents_ObjectListChanged;
        }

        private static void SaveEvents_Loaded(object sender, SaveLoadedEventArgs e)
        {
                ...
                if (location is GameLocation)
                {
                    foreach (KeyValuePair<Vector2, Object> pair in location.objects.Pairs)
                    {
                        if (location.objects[pair.Key].ParentSheetIndex == PrismaticSprinklerItem.INDEX)
                        {
                            ...
                            sprinkler.lightSource = new LightSource(LightSource.sconceLight, new Vector2((sprinkler.boundingBox.X + 32), (sprinkler.boundingBox.Y + 32)), 2.0f, Color.Black, id);
                            location.GetType().GetMethod("Add")?.Invoke(location, new[] { sprinkler.lightSource.Clone() });
        ...
        private static void LocationEvents_ObjectListChanged(object sender, ObjectListChangedEventArgs e)
        {
            ...
            foreach (KeyValuePair<Vector2, Object> pair in e.Added)
            {
                ...
                if (obj.ParentSheetIndex == PrismaticSprinklerItem.INDEX)
                {
                    ...
                    obj.lightSource = new LightSource(LightSource.sconceLight, new Vector2((obj.boundingBox.X + 32), (obj.boundingBox.Y + 32)), 2.0f, Color.Black, id);
                    obj.Name = "Prismatic Scarecrow Sprinkler";
                    Game1.currentLocation.GetType().GetMethod("Add")?.Invoke(Game1.currentLocation, new[] { obj.lightSource.Clone() });
                    //Game1.currentLocation.sharedLights.Add(obj.lightSource.Clone());
                }
            }
        }

        private static void TimeEvents_DayStarted(object sender, DayStartedEventArgs e)
        {
            foreach (GameLocation location in Game1.locations)
            {
                foreach (Object obj in location.Objects.Values)
